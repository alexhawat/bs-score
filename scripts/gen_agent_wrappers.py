...
**The skill** — clone the repository into {title}'s skills folder, so the root
`SKILL.md` and the `checklists/` it links to land together:

```bash
git clone --depth 1 https://github.com/alexhawat/bs-score {personal}   # personal
git clone --depth 1 https://github.com/alexhawat/bs-score {project}   # project
```
...
Skills folder: `{personal}` (personal) or `{project}` (project).{note_block}

```bash
git clone --depth 1 https://github.com/alexhawat/bs-score {personal}
...
def render(runtime: str) -> dict[str, str]:
    ...
    context = dict(RUNTIMES[runtime])
    note = context.pop("note", "")
    context["note_block"] = f" {note}" if note else ""
    return {
