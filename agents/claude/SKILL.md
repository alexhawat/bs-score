...
# Bullshit Score (`bs-score`) — Claude Code

## Install

Two pieces: the skill files, and the `bs-score` command that scores them.

**The skill** — clone the repository into Claude Code's skills folder, so the root
`SKILL.md` and the `checklists/` it links to land together:

```bash
git clone --depth 1 https://github.com/alexhawat/bs_score ~/.claude/skills/bs-score   # personal
git clone --depth 1 https://github.com/alexhawat/bs_score .claude/skills/bs-score   # project
```

`~/.claude/skills/bs-score` applies to every project; `.claude/skills/bs-score` is checked into one
repository. Keep the folder name `bs-score` — it is the skill's name.
