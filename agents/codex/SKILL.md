...
# Bullshit Score (`bs-score`) — Codex

## Install

Two pieces: the skill files, and the `bs-score` command that scores them.

**The skill** — clone the repository into Codex's skills folder, so the root
`SKILL.md` and the `checklists/` it links to land together:

```bash
git clone --depth 1 https://github.com/alexhawat/bs-score ~/.codex/skills/bs-score   # personal
git clone --depth 1 https://github.com/alexhawat/bs-score .agents/skills/bs-score   # project
```

`~/.codex/skills/bs-score` applies to every project; `.agents/skills/bs-score` is checked into one
repository. Keep the folder name `bs-score` — it is the skill's name. Codex also reads `~/.agents/skills/`.
